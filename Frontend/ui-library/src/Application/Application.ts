...
export class Application {
  ...
  public createButtons() {
    ...
    const controlsUIConfig: ControlsUIConfiguration = {
      statsButtonType:
          this._options.statsPanelConfig ? this._options.statsPanelConfig.visibilityButtonConfig : undefined,
    ...
    const xrButton: HTMLElement|undefined = controls.xrIcon ? controls.xrIcon.rootElement :
  ...
  configureSettings(): void {
    ...
    this.configUI.addCustomFlagOnSettingChangedListener(LightMode, (isLightMode: boolean) => {
      this.configUI.setCustomFlagLabel(LightMode, `Color Scheme: ${isLightMode ? 'Light' : 'Dark'} Mode`);
  ...
  registerCallbacks() {
    ...
    this.stream.addEventListener('playStreamRejected', ({data:
                                                             {reason}}) => this.onPlayStreamRejected(reason));
    this.stream.addEventListener(
        'loadFreezeFrame',
  ...
  /**
   * Event fired when the video is disconnected - displays the error overlay and resets the buttons stream
   * tools upon disconnect
   * @param eventString - the event text that will be shown in the overlay
   * @param allowClickToReconnect - true if we want to allow the user to click to reconnect. Otherwise it's
   *     just a message.
   */
