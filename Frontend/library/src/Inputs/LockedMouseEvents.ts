...
export class LockedMouseEvents implements IMouseEvents {
    ...
    lockStateChange() {
        ...
        if (document.pointerLockElement === videoElementParent ||
            ...
            // This is necessary as when the mouse loses focus, the windows stops listening for events and as
            // such the keyup listener won't get fired
    ...
    handleMouseDown(mouseEvent: MouseEvent) {
        ...
        toStreamerHandlers.get('MouseDown')([
            mouseEvent.button,
            // We use the store value of this.coord as opposed to the mouseEvent.x/y as the mouseEvent
            // location uses the system cursor location which hasn't moved
            this.coord.x,
            this.coord.y
        ]);
    }
