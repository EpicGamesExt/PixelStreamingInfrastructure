  ...
  updateStatus() {
    ...
    for (const controller of this.controllers) {
      ...
      for (let i = 0; i < currentState.axes.length; i += 2) {
        ...
        // https://w3c.github.io/gamepad/#remapping Gamepad browser side standard mapping has positive down,
        // negative up. This is downright disgusting. So we fix it.
        const y = -parseFloat(currentState.axes[i + 1].toFixed(4));

        // UE's analog axes follow the same order as the browsers, but start at index 1 so we will offset as
        // such
        ...
        toStreamerHandlers.get('GamepadAnalog')([
          ...
          i + 2,
          y
        ]);  // Vertical axes, offset by two (1 to match UEs axes convention and then another 1 for the
             // vertical axes)
      }
