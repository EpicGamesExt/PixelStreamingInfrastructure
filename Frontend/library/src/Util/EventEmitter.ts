...
export class WebRtcDisconnectedEvent extends Event {
  ...
  readonly data: {
    ...
    eventString: string;
    /**
       true if the user is able to reconnect, false if disconnected because of unrecoverable reasons like not
       able to connect to the signaling server
     */
...
/**
 * An event that is emitted when receiving UnfreezeFrame message from UE and video playback is about to be
 * resumed.
 */
...
export class StreamerListMessageEvent extends Event {
  ...
  readonly data: {
    ...
    /**
       Auto-selected streamer from the list, or null if unable to auto-select and user should be prompted to
       select
     */
