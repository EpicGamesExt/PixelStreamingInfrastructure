...
export class PixelStreaming {
  ...
  private configureSettings(): void {
    this.config._addOnSettingChangedListener(Flags.IsQualityController, (wantsQualityController: boolean) => {
    ...
    this.config._addOnOptionSettingChangedListener(OptionParameters.PreferredCodec, (newValue: string) => {
  ...
  /**
   * Instantiate the WebRTCPlayerController interface to provide WebRTCPlayerController functionality within
   * this class and set up anything that requires it
  ...
  _setupWebRtcTCPRelayDetection(statsReceivedEvent: StatsReceivedEvent) {
    ...
    if (activeCandidatePair != null) {
      ...
      if (localCandidate != null && localCandidate.candidateType == 'relay' &&
   ...
   * @param listener event handler function
   */
  public removeEventListener<T extends PixelStreamingEvent['type'], E extends PixelStreamingEvent&{type: T}>(
      type: T,
      listener: (e: Event&E) => void) {
