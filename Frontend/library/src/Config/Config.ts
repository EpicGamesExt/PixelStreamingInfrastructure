...
export class Config {
  ...
  _addOnOptionSettingChangedListener(id: OptionParametersIds, onChangedListener: (newValue: string) => void):
      void {
    ...
    for (const key of this.numericParameters.keys()) {
      ...
      if (number) {
        number.onChangeEmit = (newValue: number) => eventEmitter.dispatchEvent(
            new SettingsChangedEvent({id: number.id, type: 'number', value: newValue, target: number}));
      }
    }
    ...
    for (const key of this.optionParameters.keys()) {
      ...
      if (option) {
        option.onChangeEmit = (newValue: string) => eventEmitter.dispatchEvent(
            new SettingsChangedEvent({id: option.id, type: 'option', value: newValue, target: option}));
      }
    }
  }
}
