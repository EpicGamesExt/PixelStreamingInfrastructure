...
export class LatencyCalculator {
	...
	public calculate(stats: AggregatedStats, receivers: RTCRtpReceiver[]) : LatencyInfo {
		let latencyInfo = new LatencyInfo();

		let activeCandidatePair: CandidatePairStats = stats.getActiveCandidatePair();

		if(activeCandidatePair !== null && activeCandidatePair.currentRoundTripTime !== undefined && activeCandidatePair.currentRoundTripTime > 0) {
			// Get RTT
			...
			let captureSource: RTCRtpCaptureSource = this.getCaptureSource(receivers);
		...
		if(stats.inboundVideoStats.totalProcessingDelay !== undefined && stats.inboundVideoStats.framesDecoded !== undefined) {
			latencyInfo.AverageProcessingDelayMs = (stats.inboundVideoStats.totalProcessingDelay / stats.inboundVideoStats.framesDecoded) * 1000;
		}

		// https://w3c.github.io/webrtc-stats/#dom-rtcinboundrtpstreamstats-jitterbufferminimumdelay
		if(stats.inboundVideoStats.jitterBufferDelay !== undefined && stats.inboundVideoStats.jitterBufferEmittedCount !== undefined) {
			latencyInfo.AverageJitterBufferDelayMs = (stats.inboundVideoStats.jitterBufferDelay / stats.inboundVideoStats.jitterBufferEmittedCount) * 1000;
		}

		// https://w3c.github.io/webrtc-stats/#dom-rtcinboundrtpstreamstats-totaldecodetime
		if(stats.inboundVideoStats.framesDecoded !== undefined && stats.inboundVideoStats.totalDecodeTime !== undefined) {
			latencyInfo.AverageDecodeLatencyMs = (stats.inboundVideoStats.totalDecodeTime / stats.inboundVideoStats.framesDecoded) * 1000;
		}

		// https://w3c.github.io/webrtc-stats/#dom-rtcinboundrtpstreamstats-framesassembledfrommultiplepackets
		if(stats.inboundVideoStats.totalAssemblyTime !== undefined && stats.inboundVideoStats.framesAssembledFromMultiplePackets !== undefined) {
			latencyInfo.AverageAssemblyDelayMs = (stats.inboundVideoStats.totalAssemblyTime / stats.inboundVideoStats.framesAssembledFromMultiplePackets) * 1000;
		}
		...
		if(latencyInfo.AverageProcessingDelayMs !== undefined && latencyInfo.AverageProcessingDelayMs > 0 &&
		   latencyInfo.SenderLatencyMs != undefined && latencyInfo.SenderLatencyMs > 0 &&
		   latencyInfo.RTTMs != undefined && latencyInfo.RTTMs > 0) {
	...
	private calculateSenderLatency(stats: AggregatedStats, captureSource: RTCRtpCaptureSource) : number {
		...
		let senderCaptureTimestamp = captureSource.captureTimestamp + captureSource.senderCaptureTimeOffset;
		...
		let recvCaptureTimestampNTP = senderCaptureTimestamp + sendRecvClockOffset;
		...
		let recvCaptureTimestamp = recvCaptureTimestampNTP - ntp1970;

		let senderLatency =  captureSource.timestamp - recvCaptureTimestamp;
	...
	private getCaptureSource(receivers: RTCRtpReceiver[]) : RTCRtpCaptureSource {
		...
		for(let receiver of receivers) {
			...
			let sources : any[] = receiver.getSynchronizationSources().concat(receiver.getContributingSources());

			for(let src of sources) {
				if(src.captureTimestamp !== undefined && src.senderCaptureTimeOffset !== undefined && src.timestamp !== undefined) {
					let captureSrc = new RTCRtpCaptureSource();
	...
	private calculateSenderReceiverClockOffset(stats: AggregatedStats) {
		...
		let remoteVideoStatsArrivedTimestamp = stats.outBoundVideoStats.timestamp;
		let remoteVideoStatsSentTimestamp = stats.outBoundVideoStats.remoteTimestamp;

		let activeCandidatePair: CandidatePairStats = stats.getActiveCandidatePair();
		let networkDelay = activeCandidatePair ? activeCandidatePair.currentRoundTripTime * 0.5 * 1000 : 0.0;

		if(remoteVideoStatsArrivedTimestamp !== undefined && remoteVideoStatsSentTimestamp !== undefined && networkDelay !== undefined) {
