...
export class PeerConnectionController {
  ...
  handleOnIce(iceCandidate: RTCIceCandidate) {
    ...
    if (this.config.isFlagEnabled(Flags.ForceTURN)) {
      ...
      if (iceCandidate.candidate.indexOf('relay') < 0) {
        Logger.Info(
            Logger.GetStackTrace(),
            `Dropping candidate because it was not TURN relay. | Type= ${iceCandidate.type} | Protocol= ${
                iceCandidate.protocol} | Address=${iceCandidate.address} | Port=${iceCandidate.port} |`,
            6);
        return;
      }
    }
  ...
  /**
   * Find the intersection between UE and browser codecs, with fuzzy matching if some parameters are
   * mismatched.
  ...
  fuzzyIntersectUEAndBrowserCodecs(sdp: RTCSessionDescriptionInit): string[] {
    ...
    for (const ueCodec of allUECodecs) {
      ...
      // Otherwise check if browser codec at least contains a match for the UE codec name (without
      // parameters).
      else {
              ...
              (browserSupportedCodec: RTCRtpCodecCapability) => {
                ...
                if (browserSupportedCodec.mimeType != preferredRTCRtpCodecCapability.mimeType) {
                  ourSupportedCodecs.push(browserSupportedCodec);
                    ...
                    browserSupportedCodec?.sdpFmtpLine != preferredRTCRtpCodecCapability?.sdpFmtpLine) {
                  ourSupportedCodecs.push(browserSupportedCodec);
                }
              });
  ...
  parseAvailableCodecs(rtcSessionDescription: RTCSessionDescriptionInit): Array<string> {
    ...
    sections.forEach((mediaSection) => {
      ...
      codecs.forEach((c) => {
        const str =
            c.name + ' ' + Object.keys(c.parameters || {}).map((p) => p + '=' + c.parameters[p]).join(';');
