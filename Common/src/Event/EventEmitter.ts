...
export class EventEmitter extends EventTarget {
	...
	private removeListenerInternal(eventName: string, listener: Function) : this {
		if(this._eventListeners.has(eventName)) {
			...
			for(let i = 0; i < listeners.length; ++i) {
				let eventPair: EventListenerPair = listeners[i];
	...
	on(eventName: string, listener: Function): this {
		...
		this._eventListeners.get(eventName)?.push({ callback: listener, eventListenerWrapper: eventListenerWrapper })
	...
	once(eventName: string, listener: Function): this {
		...
		const eventListenerWrapper: PixelStreamingEventListener = new PixelStreamingEventListener((...args : any[]) => {
			listener(args);
	...
	removeAllListeners(eventName: string): this {
		if(this._eventListeners.has(eventName)){
			...
			for(let listenerPair of listeners) {
	...
	emit(eventName: string, ...args: any[]): boolean {
		if(this._eventListeners.has(eventName)){
			...
			for(let listenerPair of listeners) {
				listenerPair.eventListenerWrapper.args = args;
			}
